...
BDD_NODE *bdd_zoom(BDD_NODE *node, int level, int factor) {
    ...
    if (f < 0 && level+f <= 0) {
        ...
        left = bdd_zoom(bdd_nodes+left, level-1, factor)-bdd_nodes;
        right = bdd_zoom(bdd_nodes+right, level-1, factor)-bdd_nodes;
        // debug("%i %i %i %i", current_level+f, left, right, bdd_lookup(current_level+f, left, right));
        return bdd_nodes+bdd_lookup(level+f, left, right);
    }
   return NULL;
}
