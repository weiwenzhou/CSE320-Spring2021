...
/* successor of state */ // what is this???
...
#ifdef __STDC__
static int ispiece(char c) // validate the piece
#else
...
#endif
{
  ...
  if ( mo->next->whiteturn) { // if next move is white increment counter
    mo->next->move++;
  }
...
#endif
{
  ...
  if ( c1 == c2 ) {    // letter(col) is equal check rank (row)
    col = c1;
  ...
  if ( l1 == l2) { // rank is equal check letter/col
    lig = l1 ;
...
#endif
{
  ...
  for (i=0; i< nb; i++ ) {
    l = l2 - tab[i][0];
    c = c2 - tab[i][1];
    if (in_board(l,c)) // if in the board
      if ((tos->board[l][c] == m->piece) &&
	  (tos->color[l][c] == CURCOLOR(m)) &&
	  ( !path || (path && path_free(l,c, l2, c2))) &&
	  ( ((*pl1) == 0) || ((*pl1) == l) ) &&
	  ( ((*pc1) == 0) || ((*pc1) == c) ) ) // if is the correct piece, same color, and not e.p or (is path and path is free) and pl1/pc1 is not 0 or current position
	{
...
#ifdef __STDC__
int check_move(depl *m) // verify the move
#else
int check_move(m)
     depl * m;
#endif
{
  ...
  if ((tos->board[l1][c1] != m->piece)||
      (tos->color[l1][c1] != CURCOLOR(m))){ // wrong piece
  ...
  /* prendre une de ses propres pieces */ // take your own color piece
...
/* configure the board */ // ??? something with pawns
#ifdef __STDC__
int configure(void)
#else
int configure()
...
/* execute a move, no checking */ // does the move in m and doesn't care
#ifdef __STDC__
...
#endif
{
  ...
  if (m->piece == VOID ) // set the pawn
    m->piece = PAWN;

  if ((m->fromlig == 0) || (m->fromcol == 0)) // out of bounds
    (void) guess_move();
  
  /* supply to the -- maybe -- deficiency of input notation
     */
  if ((m->fromlig !=0) || (m->fromcol != 0)) // get the piece to move if --
    m->piece = tos->board[m->fromlig][m->fromcol];

  if (tos->board[m->tolig][m->tocol] != VOID) { // if new pos. is not empty, it's a capture
    m->type = PRISE;
    m->prise = tos->board[m->tolig][m->tocol] ;
  }

  if (!check_move(m)) { // if invalid move
    if (m->whiteturn)
  ...
  /* do we need to display the move ? */ // unsure???
  if (nb_move_to_dsp > 0) {
...
#ifdef __STDC__
int typechar(char c) // check what c is
#else
int typechar(c)
     char c;
#endif
{
  if (ispiece(c)) // piece
    return(0);
  if ((c >=  'a') && ( c <= 'h')) // letter/col
    return(1);
  if ((c >=  '1') && ( c <= '8')) // rank/row
    return(2);
  if ( c== '-' ) // a dash
    return(3);
  if ((c == 'x') || (c == 'X' )) // a capture
    return(4);
  if (c == '=' ) // promotion
    return(5);
  if (c == '\0' ) // EOS
...
{
  switch (num) {
  case 1: /* set cur piece */ // a piece
    curpiece = piece(c);
    break;
  case 2: /* set cur col */ // letter/col
    curcol = lettertocol(c);
    break;
  case 3: /* set cur lig */ // rank/row
    curlig = lettertolig(c);
    break;
  case 4: /* from = cur ; prise = false */ // a move
    m->piece = curpiece ;
    m->fromcol = curcol ;
    m->fromlig = curlig;
    /*m->topiece = curpiece;*/
    break;
  case 5: /* from = cur ; prise = true */ // a capture
    m->piece = curpiece ;
    m->fromcol = curcol ;
    m->fromlig = curlig;
    m->type = PRISE ;
    m->prise = curpiece;
    break;
  case 6: /* to = cur ; guess from */ // to current but no clue where from
...
#ifdef __STDC__
int parse_number(char *token) // move number parser
#else
...
#ifdef __STDC__
int parse_text(char *text) // short text?
#else
int parse_text(text)
     char *text;
#endif
...
#ifdef __STDC__
int parse_keyword(char *token, char *text) // the @s 
#else
...
#ifdef __STDC__
int parse_roque(char *token) // castling
#else
...
{
  register int i;
  int correcte = FALSE ; // correct
  int erreursyntaxe = FALSE ; // syntax error
  int etat =0; // state
  int code;
...
#ifdef __STDC__
void init_parse(depl *m) // 0 everything
#else
void init_parse(m)
     depl * m ;
#endif
...
/* cette fonction analyse les arguments de la ligne de commande
   */
#ifdef __STDC__
int parse_options(int argc,char *argv[]) // parse command line
#else
...
#endif
{
  ...
  int i;
  register int c;
  char cp[132]; // buffer of comparison
  char chaine[MAXTOKLEN]; // chain?
